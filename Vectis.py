...
if player_obj:
    try:
        # Se asume temporada actual. NBA API requiere el formato 'YYYY-YY'
        ...
        if not df.empty:
            ...
            # Obtener abreviatura del equipo actual desde el log de partidos
            equipo = df.iloc[0]['TEAM_ABBREVIATION'] if 'TEAM_ABBREVIATION' in df.columns else "N/A"

            # Bloque Pick Dinámico con el equipo incluido
            pick_text = f'''
            <div style="background-color: #1e1e1e; padding: 20px; border-radius: 15px; border: 1px solid #e41b13; margin-bottom: 2

...
def get_nba_standings():
    try:
        std = leaguestandings.LeagueStandings().get_data_frames()[0]
        dict_std = {}
    ...
    if p_obj:
        try:
            ...
            if not df.empty:
                ...
                eq = df.iloc[0]['TEAM_ABBREVIATION'] if 'TEAM_ABBREVIATION' in df.columns else "N/A"
                rec = stds.get(eq, "N/A")
                ...
                if st.button(f"➕ Añadir pick a la Calculadora"):
                    st.session_state.calculadora.append({"j": full_n, "m": m_vis, "l": l_ap, "p": p_i, "w": rec})
                    st.success("¡Añadido!")
